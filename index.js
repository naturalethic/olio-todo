...
(function(){
  ...
  (function(){
    var viewFunction, prototype;
    viewFunction = function template(locals) { var buf = []; var jade_mixins = {}; var jade_interp; ;var locals_for_with = (locals || {});(function (completed, editing, items, newTodo, route, title, undefined) { var jade_indent = []; buf.push("\n<section class=\"todoapp\">\n <header class=\"header\">\n <h1>todos</h1>\n <input autofocus=\"autofocus\" placeholder=\"What needs to be done?\"" + (jade.attr("value", (newTodo), true, false)) + " class=\"new-todo\"/>\n </header>"); if ( items.length) { buf.push("\n <section class=\"main\">\n <input type=\"checkbox\"" + (jade.attr("checked", (!(items.length - completed)), true, false)) + " class=\"toggle-all\"/>\n <label for=\"toggle-all\">Mark all as complete</label>\n <ul class=\"todo-list\">"); ;(function(){ var $$obj = items; if ('number' == typeof $$obj.length) { for (var index = 0, $$l = $$obj.length; index < $$l; index++) { var item = $$obj[index]; buf.push("\n <li" + (jade.attr("title", (item.title), true, false)) + (jade.cls(['' + (item.completed ? "completed" : "") + ' ' + (item.hidden ? "hidden" : "") + ' ' + (editing == index ? "editing" : "") + ''], [true])) + ">\n <div class=\"view\">\n <input type=\"checkbox\"" + (jade.attr("checked", (item.completed), true, false)) + " class=\"toggle\"/>\n <label>" + (jade.escape(null == (jade_interp = item.title) ? "" : jade_interp)) + "</label>\n <button class=\"destroy\"></button>\n </div>\n <input" + (jade.attr("value", (title), true, false)) + " class=\"edit\"/>\n </li>"); } } else { var $$l = 0; for (var index in $$obj) { $$l++; var item = $$obj[index]; buf.push("\n <li" + (jade.attr("title", (item.title), true, false)) + (jade.cls(['' + (item.completed ? "completed" : "") + ' ' + (item.hidden ? "hidden" : "") + ' ' + (editing == index ? "editing" : "") + ''], [true])) + ">\n <div class=\"view\">\n <input type=\"checkbox\"" + (jade.attr("checked", (item.completed), true, false)) + " class=\"toggle\"/>\n <label>" + (jade.escape(null == (jade_interp = item.title) ? "" : jade_interp)) + "</label>\n <button class=\"destroy\"></button>\n </div>\n <input" + (jade.attr("value", (title), true, false)) + " class=\"edit\"/>\n </li>"); } } }).call(this); buf.push("\n </ul>\n </section>\n <footer class=\"footer\"><span class=\"todo-count\"><strong>" + (jade.escape((jade_interp = items.length - completed) == null ? '' : jade_interp)) + "</strong> item" + (jade.escape((jade_interp = (items.length - completed) != 1 ? 's' : '') == null ? '' : jade_interp)) + " left</span>\n <ul class=\"filters\">\n <li><a href=\"#/\"" + (jade.cls(['' + (!route ? "selected" : "") + ''], [true])) + ">All</a></li>\n <li><a href=\"#/active\"" + (jade.cls(['' + (route == "active" ? "selected" : "") + ''], [true])) + ">Active</a></li>\n <li><a href=\"#/completed\"" + (jade.cls(['' + (route == "completed" ? "selected" : "") + ''], [true])) + ">Completed</a></li>\n </ul>"); if ( completed) { buf.push("\n <button class=\"clear-completed\">Clear completed</button>"); } buf.push("\n </footer>"); } buf.push("\n</section>\n<footer class=\"info\">\n <p>Double-click to edit a todo</p>\n <p>Template by&nbsp;<a href=\"http://sindresorhus.com\">Sindre Sorhus</a></p>\n <p>Created by&nbsp;<a href=\"http://todomvc.com\">@naturalethic</a></p>\n <p>Part of&nbsp;<a href=\"http://todomvc.com\">TodoMVC</a></p>\n</footer>");}.call(this,"completed" in locals_for_with?locals_for_with.completed:typeof completed!=="undefined"?completed:undefined,"editing" in locals_for_with?locals_for_with.editing:typeof editing!=="undefined"?editing:undefined,"items" in locals_for_with?locals_for_with.items:typeof items!=="undefined"?items:undefined,"newTodo" in locals_for_with?locals_for_with.newTodo:typeof newTodo!=="undefined"?newTodo:undefined,"route" in locals_for_with?locals_for_with.route:typeof route!=="undefined"?route:undefined,"title" in locals_for_with?locals_for_with.title:typeof title!=="undefined"?title:undefined,"undefined" in locals_for_with?locals_for_with.undefined:typeof undefined!=="undefined"?undefined:undefined));;return buf.join(""); };
    prototype = Object.create(HTMLElement.prototype);
    prototype.attachedCallback = function(){
      var this$ = this;
      m.render(this, eval(m.convert(viewFunction(this.start()))));
      s.merge(this.intent()).map(function(it){
        info('INTENT', it);
        return this$.model(it);
      }).onValue(function(it){
        info('MODEL', it);
        return m.render(this$, eval(m.convert(viewFunction(it))));
      });
      return this.ready();
    };
    prototype.start = function (){var model;if (model = localStorage.getItem('todomvc-olio')) {return JSON.parse(model);} else {return {newTodo: '',items: [],completed: 0};}};
    prototype.intent = function (){return [s.fromChildEvents(this, 'keydown', '.new-todo').filter(function(it){return it.which === 13 && it.target.value;}).map(function(it){return {newTodo: it.target.value.trim()};}), s.fromChildEvents(this, 'click', '.toggle').map(function(it){return {toggle: it.currentTarget.parentElement.parentElement};}), s.fromChildEvents(this, 'click', '.toggle-all').map(function(it){return {toggleAll: it.target.checked};}), s.fromChildEvents(this, 'click', '.clear-completed').map(function(){return {clearCompleted: true};}), s.fromChildEvents(this, 'click', '.destroy').map(function(it){return {destroy: it.currentTarget.parentElement.parentElement};}), s.fromChildEvents(this, 'dblclick', '.view label').map(function(it){info(setTimeout(function(){return q('.edit').focus().select();}));return {editing: q(it.currentTarget.parentElement.parentElement).index()};}), s.fromChildEvents(this, 'blur', '.edit'), s.fromChildEvents(this, 'keydown', '.edit').filter(function(it){return it.which === 13;}).map(function(it){return {editingComplete: q(it.currentTarget.parentElement).index(),title: it.target.value};}), s.fromChildEvents(this, 'keydown', '.edit').filter(function(it){return it.which === 27;}).map(function(it){return {editingComplete: q(it.currentTarget.parentElement).index()};}), s.fromChildEvents(this, 'input', '.edit').map(function(it){return {editing: q(it.currentTarget.parentElement).index(),title: it.target.value};}), s.fromEvents(q(window), 'route').map(function(it){return {route: it};})];};
    prototype.model = function (intent){var model;model = {editing: intent.editing,newTodo: (!intent.newTodo && q(this).find('.new-todo').val()) || '',items: each(function(it){return it.hidden = (currentRoute() === 'active' && it.completed) || (currentRoute() === 'completed' && !it.completed);})(filter(function(it){return !(intent.clearCompleted && it.completed);})(map(function(it){intent.toggleAll != null && (it.completed = intent.toggleAll);return it;})(map(function(it){return {title: (intent.itemEdit === it && intent.itemEditText) || q(it).attr('title'),completed: q(it).hasClass('completed') === (it !== intent.toggle),edit: it === intent.itemWantsEdit};})(filter(function(it){return intent.destroy !== it;})(q(this).find('.todo-list li'))).concat((intent.newTodo && {title: intent.newTodo,completed: false}) || []))))};model.completed = filter(function(it){return it.completed;})(model.items).length;if (intent.editing != null) {intent.title == null && (intent.title = model.items[intent.editing].title);model.title = intent.title;}if (intent.editingComplete != null) {if (intent.title != null) {model.items[intent.editingComplete].title = intent.title;}}localStorage.setItem('todomvc-olio', JSON.stringify(model));if (intent.route != null) {model.route = intent.route;}return model;};
    prototype.module = function fn$(){};
    prototype.ready = function fn1$(){};
    return document.registerElement('olio-todo', {
      prototype: prototype
    });
  })();
  function clone$(it){
